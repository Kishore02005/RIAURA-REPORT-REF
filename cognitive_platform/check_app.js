...
if(dbEl){
  ...
  var W=1200,H=440,padL=40,padR=40,padT=32,padB=80;
  ...
  svg.setAttribute('viewBox','0 0 '+W+' '+H);svg.setAttribute('preserveAspectRatio','xMinYMid meet');
  ...
  sorted.forEach(function(dd,i){
    ...
    var lbl=document.createElementNS(ns,'text');lbl.setAttribute('x',x);lbl.setAttribute('y',H-padB+14);lbl.setAttribute('text-anchor','middle');lbl.classList.add('dc-axis-label');lbl.textContent=shortNames[dd.name]||dd.name;svg.appendChild(lbl);
    [25,50,75].forEach(function(tv){
...
/* Appendix — Brain Regions grid */
var brEl=document.getElementById('app-brain-grid');
if(brEl){d.glossary.forEach(function(g){var c=document.createElement('div');c.className='abcard';c.innerHTML='<div class="abcard__img"><img src="'+g.brain_image+'" alt="'+g.name+' brain region" loading="lazy"></div><div class="abcard__accent" style="background:'+g.color+'"></div><div class="abcard__body"><span class="abcard__name">'+g.name+'</span><span class="abcard__region">'+g.brain_region+'</span><p class="abcard__desc">'+g.description+'</p></div>';brEl.appendChild(c);});}

/* Appendix — Glossary */
var glEl=document.getElementById('app-glossary');
if(glEl){var gh='';d.glossary.forEach(function(g){gh+='<div class="agloss"><span class="agloss__dot" style="background:'+g.color+'"></span><div class="agloss__text"><span class="agloss__name">'+g.name+'</span><p class="agloss__desc">'+g.description+'</p></div></div>';});glEl.innerHTML=gh;}

/* Appendix — Colour System */
var clEl=document.getElementById('app-colors');
if(clEl){var ch='';d.glossary.forEach(function(g){ch+='<div class="acolor"><span class="acolor__swatch" style="background:'+g.color+'"></span><span class="acolor__name">'+g.name+'</span><span class="acolor__hex">'+g.color+'</span></div>';});clEl.innerHTML=ch;}
...
/* Manually trigger stagger for dynamically-populated containers */
document.querySelectorAll('.stagger').forEach(function(el){
  if(el.children.length>0&&!el.querySelector('.is-on')){
    var k=el.children;for(var i=0;i<k.length;i++){
    (function(c,j){setTimeout(function(){c.classList.add('is-on');},j*70);})(k[i],i);
  }}
});
})();
